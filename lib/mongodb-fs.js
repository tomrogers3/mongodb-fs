...
that = {
  ...
  init: function (config) {
    ...
    if (that.config.fork && that.child) {
      that.child.send({ action: 'init', config: JSON.parse(EJSON.stringify(that.config)) });
    }
  },
  start: function (callback) {
    ...
    if (that.config.fork) {
      ...
      that.child.send({ action: 'start', config: JSON.parse(EJSON.stringify(that.config)) });
...
if (module.parent) {
  ...
  process.on('message', function (data) {
    if (data.action === 'start') {
      ...
      that.init(EJSON.parse(JSON.stringify(data.config)));
