...
function nestedProp(x, path) {
  if (path && path.length > 0 && x) {
    return nestedProp(x[path.shift()], path);
  } else {
    return x;
  }
}
  ...
  doQuery: function (socket, clientReqMsg) {
    ...
    try {
      if (clientReqMsg.query && !helper.isEmpty(clientReqMsg.query)) {
        var q = clientReqMsg.query;
        if(clientReqMsg.query["$query"]) q = clientReqMsg.query["$query"];
        docs = filter.filterItems(collection, q);

        // process orderby clause, if it exists
        var orderby = clientReqMsg.query.orderby;
